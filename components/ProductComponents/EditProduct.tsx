...
export default function EditProduct({ productId }: { productId: number }) {
  ...
  useEffect(() => {
    ...
    const getProductById = async () => {
      try {
        ...
        setValue('productCategory', String(productData.categoryId));
        setValue('productName', productData.product_name);
        setValue('productDescription', productData.product_description);
        setValue('productPrice', String(productData.product_price));
      } catch (error) {
        setCategoryError('Failed to get categories');
      }
    };
  ...
  const onSubmit: SubmitHandler<ProductFormData> = async (data) => {
    startTransition(async () => {
      try {
        const res = await ProductUpdate(productId, {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the details of the editing product and select a category.
          </DialogDescription>
        </DialogHeader>
